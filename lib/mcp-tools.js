...
const TOOLS = [
  ...
  {
    name: 'diagnose_docusign_ids',
    description:
      'Read-only diagnostic: echoes back DOCUSIGN_INTEGRATION_KEY, DOCUSIGN_USER_ID, DOCUSIGN_ACCOUNT_ID, and ' +
      'DOCUSIGN_AUTH_BASE_URL exactly as stored — these are non-secret public identifiers (like a client_id), safe to display, ' +
      'unlike DOCUSIGN_RSA_PRIVATE_KEY. Use this to directly compare against what DocuSign\'s admin console actually shows.',
    inputSchema: { type: 'object', properties: {} },
  },
];
...
async function callTool(name, args) {
  switch (name) {
    ...
    case 'diagnose_docusign_ids': {
      // Non-secret identifiers (integration key / user id / account id are public app
      // identifiers, not credentials) — safe to echo back for direct comparison against
      // what's actually registered in DocuSign's admin console.
      return {
        integrationKey: process.env.DOCUSIGN_INTEGRATION_KEY || null,
        userId: process.env.DOCUSIGN_USER_ID || null,
        accountId: process.env.DOCUSIGN_ACCOUNT_ID || null,
        authBaseUrl: process.env.DOCUSIGN_AUTH_BASE_URL || null,
      };
    }
