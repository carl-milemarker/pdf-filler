...
const TOOLS = [
  {
    name: 'onboard_pdf',
    description:
      "Onboard a PDF into a brand-new Milemarker Workflow + Form, paired with a matching DocuSign Template. " +
      'Extracts every native form field from the PDF, creates a Milemarker Form with one field per extracted PDF field, ' +
      'creates a DocuSign Template with a matching tab per field (tabLabel == Milemarker field key), and creates the Milemarker ' +
      'Workflow that binds them. Submitting the resulting Milemarker form later pre-fills and sends the PDF via DocuSign. ' +
      'Signature/initial/date tabs are NOT auto-detected — use add_field_tab afterward to place those (and any other missed field) ' +
      'conversationally, e.g. "add a signature at the bottom of the birthday field".',
    inputSchema: {
      type: 'object',
      properties: {
