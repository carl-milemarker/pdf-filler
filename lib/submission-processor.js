...
function buildEnvelopeTabs(fields, mapping) {
  ...
  for (const [key, f] of Object.entries(fields)) {
    if (f.type === 'list' && Array.isArray(f.value)) {
      if (rowKeys[key]) {
        // label_by controls whether a matched slot's tabLabel segment is the slot's own
        // name (e.g. other_contacts_secondary_entity_contact_name) or its 1-based position
        // among `slots` (e.g. investment_experience_3_years) -- must match however the
        // template's own tabs were actually labeled for that field.
        // radio_subfields names sub-fields that are real DocuSign radioGroupTabs on the
        // template (e.g. investment_experience's years/transactions were genuine PDF radio
        // buttons), not text tabs -- submitted row data carries no per-subfield type, so this
        // has to be told explicitly per list field.
        const { key_field: keyField, slots, label_by: labelBy = 'name', radio_subfields: radioSubfields = [] } = rowKeys[key];
        for (const row of f.value) {
          const idx = slots.indexOf(row[keyField]);
          if (idx === -1) continue;
          const slotLabel = labelBy === 'index' ? String(idx + 1) : slots[idx];
          for (const [subKey, subVal] of Object.entries(row)) {
            if (subKey === keyField) continue;
            const subType = radioSubfields.includes(subKey) ? 'radiogroup' : 'text';
            emitFlat(`${key}_${slotLabel}_${subKey}`, { type: subType, value: subVal });
          }
        }
      } else if (rowCaps[key]) {
...
async function processSubmission({ tenantId, webhookBody, testMode = false }) {
        ...
        ? [
            {
              ...
              // The Advisor signs first and needs to actually review the submitted data
              // before signing, not just their own name/signature block -- send the same
              // values to the Advisor's read-only duplicate tabs (locked on the template)
              // so they see everything filled in, not a blank document.
  ...
  if (isTwoSigner) {
    // DocuSign silently drops the `value` on locked tabs when creating an envelope from a
    // template (confirmed live: the editable Client copy filled in fine, the locked Advisor
    // duplicate came back empty) -- a follow-up PUT is required to actually set them. Note
    // DocuSign renumbers recipientId by templateRoles array order at creation, not by the
    // recipientId requested (Advisor is always listed first here, so it's always '1' on the
    // resulting envelope regardless of mapping.advisor_recipient_id, which only applies to
    // the template).
    const putRes = await fetch(
      `${apiBase}/v2.1/accounts/${accountId}/envelopes/${envBody.envelopeId}/recipients/1/tabs`,
      {
        method: 'PUT',
        headers: { Authorization: `Bearer ${accessToken}`, 'Content-Type': 'application/json' },
        body: JSON.stringify({ textTabs, checkboxTabs, radioGroupTabs }),
      },
    );
    if (!putRes.ok) {
      const putBody = await putRes.json().catch(() => ({}));
      throw new Error(`Failed to set Advisor's read-only tab values: ${putRes.status} ${JSON.stringify(putBody)}`);
    }
  }
