...
async function startOnboarding({ pdfBase64, pdfUrl, documentName, workflowName, workflowDescription, siteUrl }) {
  checkRequiredEnv();
  if ((!pdfBase64 && !pdfUrl) || !documentName || !workflowName) {
    throw new Error('Required: (pdf_base64 or pdf_url), document_name, workflow_name');
  }
  if (!siteUrl) {
  ...
  const pdfBytes = await resolvePdfBytes({ pdfBase64, pdfUrl });
