...
async function startOnboarding({ pdfBase64, documentName, workflowName, workflowDescription, siteUrl }) {
  ...
  // Background Function invocations have a much smaller payload cap than regular synchronous
  // Functions (confirmed live: a ~4MB base64 PDF got a 413 invoking onboard-pdf-background
  // directly, not just via this function-to-function hop) — so the PDF bytes travel via
  // Netlify Blobs instead of the invocation body itself. The background function reads them
  // back by key and deletes the blob when it's done.
  const blobStore = getStore(PDF_BLOB_STORE);
  await blobStore.set(jobId, pdfBase64);
